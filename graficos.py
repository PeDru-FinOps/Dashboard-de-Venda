...
grafico_rec_vendedores = px.bar(
    df_vendedores[['Total']].sort_values('Total', ascending=False).head(7),
    x= 'Total',
    y = df_vendedores[['Total']].sort_values('Total', ascending=False).head(7).index,
    text_auto=True,
    title= 'Top 7 Vendedores por Receita'
)

grafico_vendas_vendedores = px.bar(
    df_vendedores[['Vendas']].sort_values('Vendas', ascending=False).head(7),
    x= 'Vendas',
    y = df_vendedores[['Vendas']].sort_values('Vendas', ascending=False).head(7).index,
    text_auto=True,
